class Autocomplete {
  ...
  getMatches (text) {
    let massive = []
    for (let elem = 0; elem < this.input.length; elem++) {
      if (
        this.input[elem].innerText.toLowerCase().includes(text.toLowerCase())
      ) {
        massive.push({
          text: this.input[elem].innerText,
          value: this.input[elem].getAttribute('value')
        })
      }
    }
    return massive
